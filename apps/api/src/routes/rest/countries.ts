...
router.get("/", zValidator("query", paginationSchema), (c) => {
  ...
  return c.json(respond(data, 100, params, "countries"));
});
...
router.get("/search", zValidator("query", z.object({ q: z.string().default("") })), (c) => {
  ...
  const data = generateCountries(params);
  return c.json(respond(data, data.length, params, "countries"));
});

router.get("/:id", (c) => {
  const id = Number(c.req.param("id"));
  const skip = Number.isFinite(id) && id > 0 ? id - 1 : 0;
  const item = generateCountries({ limit: 1, skip, order: "asc" })[0];
