...
router.get("/", zValidator("query", paginationSchema), (c) => {
  ...
  const data = generateNotifications(params);
  return c.json(respond(data, 100, params, "notifications"));
});
...
router.get("/search", zValidator("query", z.object({ q: z.string().default("") })), (c) => {
  ...
  const data = generateNotifications(params);
  return c.json(respond(data, data.length, params, "notifications"));
});

router.get("/:id", (c) => {
  const id = Number(c.req.param("id"));
  const skip = Number.isFinite(id) && id > 0 ? id - 1 : 0;
  const item = generateNotifications({ limit: 1, skip, order: "asc" })[0];
  return c.json({ data: item ?? null });
});

router.post("/", async (c) => {
  const body = await c.req.json().catch(() => ({}));
  const item = generateNotifications({ limit: 1, skip: 0, order: "asc" })[0];
